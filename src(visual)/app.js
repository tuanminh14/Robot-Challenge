...
const comTextBox = document.querySelector(".commands");

export class App {
  constructor() {
    this.robots = [];
    enterBtn.addEventListener("click", this.takingCommand.bind(this));
  }
  ...
  makeRobotActive(command) {
    ...
    if (command.includes("robot")) {
      ...
      if (this.robots[robotNumber - 1]) {
        document
          .querySelector(`.${this.activeRobot.name}`)
          .classList.remove("active");
        this.activeRobot = this.robots[robotNumber - 1];
      }
    }
    document.querySelector(`.${this.activeRobot.name}`).classList.add("active");
  }
  ...
  placeRobot(command) {
    if (this.placeCommandVal(command)) {
      ...
      if (this.placeInputValidation(comm, x, y, direction)) {
        ...
        this.robots[currentRobot].visualDisplay();
      }
    }
  }
  ...
  takingCommand() {
    let commands;
    commands = comTextBox.value.toLowerCase().split("\n");
    comTextBox.value = "";
